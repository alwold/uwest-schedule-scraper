...
describe UwestScheduleScraper, '#get_class_info' do
  ...
  it "open class shows open status" do
    ...
    scraper.get_class_status("166", "SPCH101", "1").should eq(:open)
  end
  ...
  it "returns nil for non-existent class" do
    scraper.get_class_status('163', 'REL633', '').should eq(nil)
    scraper.get_class_info('163', 'REL633', '').should eq(nil)
  end
  it "blows up with not enough params" do
    expect { scraper.get_class_info("163", "ENGL101") }.to raise_error(ArgumentError)
  end
end
